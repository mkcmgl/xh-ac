<template>
  <div class="navbar">
    ...
    <div class="right-menu">
      ...
      <span class="rightTitle" v-if="checkRole(['organization'])" @click="toOp">子链接入</span>
      <span class="round" v-if="checkRole(['organization'])"></span>
...
import { redirectIdentity, redirectOp, subchainJudgr } from '@/api/login.js'
    ...
    async logout() {
      ...
      this.$confirm('确定注销并退出系统吗？', '提示', {
        ...
        this.$store.dispatch('LogOut').then(() => {
          ...
          let url = process.env.NODE_ENV === "production" ? 'http://10.221.19.217/baas/account-center/login' : 'http://10.180.13.15:80/baas/account-center/login'
          //ok
          // let url = process.env.NODE_ENV === "production" ? 'http://10.221.19.217/baas/account-center/login' : 'http://10.180.13.238:80/baas/account-center/login'
          windows.open(url, '_self');
        })
      }).catch(() => { });
    },
    toIndex() {
      let url = process.env.NODE_ENV === "production" ? 'http://10.221.19.217/baas/account-center/index' : 'http://10.180.13.15:80/baas/account-center/index'
      // 正确
      // let url = process.env.NODE_ENV === "production" ? 'http://10.221.19.217/baas/account-center/index' : 'http://10.180.13.238:80/baas/account-center/index'
      window.open(url, '_self');
    },
    toDid() {
      let url = process.env.NODE_ENV === "production" ? 'http://10.221.19.217/baas/account-center/did' : 'http://10.180.13.15:80/baas/account-center/did'
      // 正确
