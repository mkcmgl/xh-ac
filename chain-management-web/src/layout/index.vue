<template>
  <div :class="classObj" class="app-wrapper" :style="{'--current-color': theme}">
    ...
    <div class="sidebar-top" v-if="!sidebar.hide">
      主链系统
      <el-popover placement="left-start" trigger="click" class="fixHeard" v-if="!checkRole(['subchain'])">
        ...
        <div class="showContent changeLogo3" @click="toMir" v-if="checkRole(['common', 'person', 'organization'])">
          融合标识管理平台
        </div>
...
import {redirectIdentity,redirectPublic,redirectSpv} from '@/api/login.js'
    ...
    toMir(){
      ...
      if (roleArr.includes('common')) {
        ...
        redirectIdentity(params).then(res => {
          ...
          let url = process.env.NODE_ENV === "production" ? toUrl : 'http://192.168.0.106:82/login?data=' + data + ''
          this.goTarget(url)
        })
      }
    },
